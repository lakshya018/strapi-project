        ...
        <section className="text-gray-600 body-font overflow-hidden">
          <div className="container px-5 md:py-24 py-10 mx-auto">
            <div className="lg:w-4/5 mx-auto flex flex-wrap">
              <img alt="ecommerce" className="lg:w-1/2 md:w-2/5 sm:w-1/4 w-full h-auto m-auto rounded" src={product.attributes.image.data.attributes.name} />
              <div className="lg:w-1/2 w-full lg:pl-10 lg:py-6 mt-6 lg:mt-0">
                <h2 className="text-sm title-font text-gray-500 tracking-widest">{product.attributes.brand}</h2>
                ...
                <div className="flex">
                  ...
                  <div className="flex ml-auto">
                    {/* <button className="flex mx-1 text-white bg-indigo-500 border-0 py-2 px-4 focus:outline-none hover:bg-indigo-600 rounded">Buy Now</button> */}
                    <button onClick={() => { addToCart(product.attributes.title, 1, product.attributes.price, product.attributes.image.data.attributes.name) }} className="flex mx-1 text-white bg-indigo-500 border-0 py-2 px-4 focus:outline-none hover:bg-indigo-600 rounded">Add to Cart</button>
...
export async function getServerSideProps(context) {
  ...
  let response = await fetch('http://localhost:1337/api/products?filters[slug]=' + context.query.slug + "&populate=*", {
    headers: headers
  });
